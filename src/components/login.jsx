...
function Login() {
    ...
    const handleSubmission = async (e) => {
        ...
        const response = await fetch('https://presidential-dina-critic-coder-bcfa82d2.koyeb.app/login', {
            method: 'POST',
            headers: {
                        ...
                        <div className="p-8 space-y-3 md:space-y-3 sm:p-8">
                            ...
                            <form className="space-y-4 md:space-y-4" onSubmit={handleSubmission}>
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="abc@example.com" required="" onChange={(e) => setEmail(e.target.value)} />
                                </div>
                                <div>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <a href="#" className="text-sm font-medium text-primary-600 hover:underline" onClick={() => forgot()}>Forgot password?</a>
                                </div>
                                <button type="submit" className="w-full text-white bg-[#eb5b77] bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Sign in</button>
                                <p className="text-sm font-light text-gray-500">
                                    Don’t have an account yet? <a href="/signup" className="font-medium text-primary-600 underline">Sign up</a>
                                </p>
                            </form>
                        </div>
                    </div>
